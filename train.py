...
from models.simple_convnet import simple_convnet6, simple_convnet11, simple_convnet16, nostride_simple_convnet11
from logger import report_progress, report_status, report_warning
from models.layers import ConstrainedConv2d as  ConstrainedConv2d
...
def setup_argparse():
    ...
    parser.add_argument("--lip",  
                        nargs=4, 
                        help="Lipschitz constraint in format: conv1, in block, shortcut, fc", 
                        type=float, default=[-1, -1, -1, -1])
    parser.add_argument("--dist", 
                        nargs=4,  
                        help="Distance constraint in format: conv1, in block, shortcut, fc", 
                        type=float, 
                        default=[-1, -1, -1, -1])
    ...
    parser.add_argument('--datadir',
                        help="Data directory", 
                        type=str, 
                        default = '/scratch2/fgraf/data')
    parser.add_argument('--bn', 
                        help="Use batch normalization", 
                        type=bool, 
                        default = False)
    parser.add_argument('--rand', 
                        help="Randomize training labels", 
                        type=bool, 
                        default=False)
    ...
    parser.add_argument('--arch',
                        help='Architecture of the model, either nostride_preact_resnet18 or simple_convnet6 / simple_convnet11',
                        type = str,
                        default='no_stride_preactresnet18')
    parser.add_argument('--optimizer',
                        help='Optimizer to use (SGD or Adam); for Adam, --momentum and --wd are unused!',
                        type = str,
                        default='sgd')
    # The following arguments only apply to simple_convnet
    parser.add_argument('--n_channels',
                        help='Defines the number of channels of each layer of simple_convnet',
                        type = int,
                        default=512)
...
def train_model(
    ...
    dl,         # Data loader
    optimizer,  # Optimizer
    scheduler,  # Scheduler (unused for now)
...
def main():
    ...
    ds_trn, ds_tst, dl_trn, dl_tst, num_classes = get_dataset_and_loaders(
        args.dataset, 
        args.datadir, 
        batch_size=args.bs, 
        num_workers=4, 
    ...
    if args.arch == 'no_stride_preactresnet18':        
        ...
        model = simple_convnet6(num_classes, args.n_channels).to(args.device)
    elif args.arch == 'simple_convnet11':
        model = simple_convnet11(num_classes, args.n_channels).to(args.device)
    elif args.arch == 'simple_convnet16':
        model = simple_convnet16(num_classes, args.n_channels).to(args.device)
    elif args.arch == 'nostride_simple_convnet11':
        model = nostride_simple_convnet11(
            num_classes,
            args.n_channels,
            lip_cond=args.lip,
            dist_cond=args.dist).to(args.device)
    ...
    if args.arch in ["simple_convnet"+str(i) for i in [6, 11, 16]] + ["nostride_simple_convnet"+str(i) for i in [6, 11, 16]]:
    ...
    for epoch_i in range(args.epochs):
        ...
        if epoch_i % 10 == 0 or epoch_i == args.epochs - 1:
            ...
            #writer.add_scalar('Spectral Complexity', spectral_comp, epoch_i)
    ...
    if args.n_proj>0:
        model.project(15)
    trn_acc, trn_loss, _ = eval_model(model, dl_trn, args.device, loss_fn, epoch_i)
    tst_acc, tst_loss, _ = eval_model(model, dl_tst, args.device, loss_fn, epoch_i)
